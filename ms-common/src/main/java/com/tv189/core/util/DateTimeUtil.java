...
public class DateTimeUtil {
	public static final Logger log = Logger.getLogger(DateTimeUtil.class);
	...
	public final static String FORMAT_yyyy_M = "yyyy-M";
	...
	public static String strDateConvert(String date, String format, String newFormat) {
		...
		String value = null;
		try {
			value = DateTimeUtil.date2StrDate(DateTimeUtil.str2Date(date, format), newFormat);
		} catch (Exception ex) {
		}
		return value;
	}
     ...
     * @return
     */
    public static Date appDateAddDays(Date nowDate, int day, int type){
        Calendar now = Calendar.getInstance();
        SimpleDateFormat simpledate = new SimpleDateFormat("yyyy-MM-dd");
        now.setTime(nowDate);
	...
	/**
	 * 获得指定年份月份 所有的星期段
	 * 2015年7月  result:[{start=2015-06-29, end=2015-07-05}, {start=2015-07-06, end=2015-07-12}, {start=2015-07-13, end=2015-07-19}, {start=2015-07-20, end=2015-07-26}]
	 * @param year
	 * @param month
	 * @return
	 */
	public static List<Map<String,Object>> getFirstMondayOfMonth(int year,int month){
		...
		boolean flag=month==11;//12月份特殊处理
		while (calendar.get(Calendar.MONTH) == month-1 || calendar.get(Calendar.MONTH)==month || calendar.get(Calendar.MONTH)==11-month) {
			if (calendar.get(Calendar.DAY_OF_WEEK) == Calendar.MONDAY) {
				Map<String,Object> data=new HashMap<String,Object>();
				data.put("start", format.format(calendar.getTime()));
				calendar.add(Calendar.DATE, 6);
				if(calendar.get(Calendar.MONTH)>month || (flag && calendar.get(Calendar.MONTH)==0))
					break;
				data.put("end", format.format(calendar.getTime()));
				datas.add(data);
			}
	 ...
	 * @return
	 */
	public static List<Map<String,Object>> getTwoWeeksBreakOfMonth(int year,int month){
		List<Map<String,Object>> datas=new ArrayList<Map<String,Object>>();
		month--;
		SimpleDateFormat format = new SimpleDateFormat("yyyy-MM-dd");
		Calendar calendar = Calendar.getInstance();
	 ...
	 * @return
	 */
	public static Map<String,Object> getCurrentWeek(){
		SimpleDateFormat format = new SimpleDateFormat("yyyy-MM-dd");
		Calendar calendar = Calendar.getInstance();
	 ...
	 * @return
	 */
	public static Map<String,Object> getCurrentTwoWeek(){
		SimpleDateFormat format = new SimpleDateFormat("yyyy-MM-dd");
		Calendar calendar = Calendar.getInstance();
		int day=calendar.get(Calendar.DAY_OF_WEEK);
		Map<String,Object> data=new HashMap<String,Object>();
	...
	public static Map<String,Object> getCurrentWeeks(int number){
		SimpleDateFormat format = new SimpleDateFormat("yyyy-MM-dd");
		Calendar calendar = Calendar.getInstance();
		int day=calendar.get(Calendar.DAY_OF_WEEK);
		Map<String,Object> data=new HashMap<String,Object>();
		if(day==2){
			calendar.set(Calendar.DAY_OF_WEEK,Calendar.MONDAY);
			calendar.add(Calendar.DATE, -7*number);
			data.put("start", format.format(calendar.getTime()));
			calendar.add(Calendar.DATE, 7*number-1);
			data.put("end", format.format(calendar.getTime()));
		}else{
			calendar.set(Calendar.DAY_OF_WEEK,Calendar.MONDAY);
			calendar.add(Calendar.DATE, -7 * (number-1));
	 ...
	 * 取得当月天数
	 * */
	public static int getCurrentMonthHaveDays()
	{
	 ...
	 * 得到指定月的天数
	 * */
	public static int getSpecifiedMonthHaveDays(int year, int month)
	{
		Calendar a = Calendar.getInstance();
	...
	public static String getNowYearMonth(){
		return DateTimeUtil.now2Str(DateTimeUtil.FORMAT_yyyy_MM);
	}
	...
	public static String getNextYearMonth(){
		...
		return DateTimeUtil.date2StrDate(calendar.getTime(), "yyyy-MM");
	}
	 ...
	 * @return
	 */
	public static Date getTargetDayOfHour(Date date, int hour ) {
		if (date == null) return null;
		try {
			SimpleDateFormat simpledate = new SimpleDateFormat("yyyy-MM-dd");
			String dateStr = simpledate.format(date);
			dateStr += " "+hour+":00:00";
			SimpleDateFormat simpletime = new SimpleDateFormat(FORMAT_yyyy_MM_dd_HH_mm_ss);
	...
	/**
	 * 获得间隔天数的时间
	 * @param date 正数表示后几天 负数表示前几天
	 * @param day
	 * @return
	 */
	public static Date getIntervalDateOfDay(Date date, int day) {
		Calendar calendar = Calendar.getInstance();
		calendar.setTime(date);
